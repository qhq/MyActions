...
*/
const $ = new Env('城城分现金')
var cookie = $.getdata('qhq_jd_ck');
var code = $.getdata('qhq_withDrawal');
...
!(async () => {
  ...
  for (let i = 1; i <= times; i++) {
    ...
    if ($.data.data.bizMsg === 'success') {
      console.log(JSON.stringify($.data))
      $.msg($.name, '', `${$.data.data.result.desc}`)
      break;
    }
...
function taskPostUrl(functionId, body) {
  return {
    ...
    body: `functionId=city_withdraw&body={ "channel": 1, "code": "asdasdsadasdsad" }&client=wh5&clientVersion=1.0.0`,
    headers: {
      'Cookie': cookie,
      'Host': 'api.m.jd.com',
...
function getCode() {
  if ($request.method != 'OPTIONS' && $request.body.indexOf("city_withDrawal") > -1) {
    body = $request.body
    //console.log(body)
    body = decodeURIComponent(body.match(/(\{.*?\})/g));
    code = JSON.parse(body).lbsCity;
    if (code != '') {
      $.setdata(code, "qhq_withDrawal");
      //$.msg($.name, '', `本次提现码：${code}`)
    }
  }
}
