...
[task_local]
#获取互助码
11 9 * * * https://gitee.com/lxk0301/jd_scripts/raw/master/jd_get_share_code.js, tag=获取互助码, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
                    ...
                    $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/ddfactory/${item.assistTaskDetailVo.taskToken}`).then((text) => (console.log(text)));
					$.wait(2000);
				  }
                });
              }
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
                  ...
                  $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/jxfactory/${data.user.encryptPin}`).then((text) => (console.log(text)));
				  $.wait(2000);
                }
              } else {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                if (data.active) {
                  ...
                  $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/jxnc/` + JSON.stringify(shareCodeJson)).then((text) => (console.log(text)));
				  $.wait(2000);
                } else {
      ...
      try {
        if (err) {
          ...
          if (
            ...
            $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/pet/${$.petInfo.shareCode}`).then((text) => (console.log(text)));
			$.wait(2000);
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if ($.taskList.filter(item => !!item && item['taskId'] === 3) && $.taskList.filter(item => !!item && item['taskId'] === 3).length) {
                ...
                $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/jdzz/${$.taskList.filter(item => !!item && item['taskId'] === 3)[0]['itemId']}`).then((text) => (console.log(text)))
				$.wait(2000);
			  }
            }
          }
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0") {
      ...
      $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/bean/${$.myPlantUuid}`).then((text) => (console.log(text)));
	  $.wait(2000);
    } else {
      console.log(
...
async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/farm/${$.farmInfo.farmUserPro.shareCode}`).then((text) => (console.log(text)));
	  $.wait(2000);
    } else {
      /*console.log(
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data && data.data.userInviteCode) {
              ...
              $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/jdcrazyjoy/${data.data.userInviteCode}`).then((text) => (console.log(text)));
			  $.wait(2000);
            }
          }
        }
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (data.data.bizCode === 0) {
            ...
            $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/sgmh/${invites && invites[0]['assistTaskDetailVo']['taskToken']}`).then((text) => (console.log(text)));
			$.wait(2000);
          }
        } catch (e) {
          $.logErr(e, resp);
  ...
  return new Promise(async (resolve) => {
    $.get(taskUrl(`user/QueryUserInfo`), (err, resp, data) => {
      try {
        ...
        $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/cfd/${strMyShareId}`).then((text) => (console.log(text)));
		$.wait(2000);
      } catch (e) {
        $.logErr(e, resp);
      } finally {
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 0 && data.data.result) {
              ...
              $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/jdcash/${data.data.result.inviteCode}`).then((text) => (console.log(text)));
			  $.wait(2000);
            }
          }
        }
          ...
          if (safeGet(data)) {
            ...
            if (taskId === 6) {
              if (data?.data?.result?.taskVos) {
                ...
                $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/health/${data?.data?.result?.taskVos[0].assistTaskDetailVo.taskToken}`).then((text) => (console.log(text)));
				$.wait(2000);
			  }
            }
          }
      ...
      try {
        if (err) {
          ...
          if (data.code === 200) {
			...
			$.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/carnivalcity/${data.data.shareId}`).then((text) => (console.log(text)));
			$.wait(2000);
