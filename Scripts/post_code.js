              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
					...
					$.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/ddfactory/${item.assistTaskDetailVo.taskToken}`)
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
				  ...
				  $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/jxfactory/${data.user.encryptPin}`)
					.then((text) => (console.log(text)))
                }
              } else {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                if (data.active) {
				  ...
				  $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/jxnc/`+JSON.stringify(shareCodeJson))
					.then((text) => (console.log(text)))
                } else {
        ...
        if (err) {
          ...
          if (
            ...
            console.log(
			 ...
			 $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/pet/${$.petInfo.shareCode}`)
					.then((text) => (console.log(text)))
            ...
            if (safeGet(data)) {
              ...
              if ($.taskList.filter(item => !!item && item['taskId']=== 3) && $.taskList.filter(item => !!item && item['taskId']=== 3).length) {
                console.log(`【京东账号${$.index}（${$.UserName}）的京东赚赚好友互助码】${$.taskList.filter(item => !!item && item['taskId']=== 3)[0]['itemId']}`);
				 $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/jdzz/${$.taskList.filter(item => !!item && item['taskId']=== 3)[0]['itemId']}`)
					.then((text) => (console.log(text)))
              }
            }
          }
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
	...
	$.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/bean/${$.myPlantUuid}`)
					.then((text) => (console.log(text)))
    } else {
      console.log(
...
$.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/farm/${$.farmInfo.farmUserPro.shareCode}`)
					.then((text) => (console.log(text)))
    } else {
      /*console.log(
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data && data.data.userInviteCode) {
			  ...
			  $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/jdcrazyjoy/${data.data.userInviteCode}`)
					.then((text) => (console.log(text)))
            }
          }
        }
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (data.data.bizCode === 0) {
			...
			$.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/sgmh/${invites && invites[0]['assistTaskDetailVo']['taskToken']}`)
					.then((text) => (console.log(text)))
          }
        } catch (e) {
          $.logErr(e, resp);
          ...
          if (safeGet(data)) {
            ...
            if (taskId === 6) {
              if (data?.data?.result?.taskVos) {
                console.log(`\n【京东账号${$.index}（${$.UserName}）的${$.name}京东健康好友互助码】${data?.data?.result?.taskVos[0].assistTaskDetailVo.taskToken}\n`);
                // console.log('好友助力码：' + data?.data?.result?.taskVos[0].assistTaskDetailVo.taskToken)
				 $.getScript(`http://xinhunshang.xyz:6001/submit_activity_codes/halth/${data?.data?.result?.taskVos[0].assistTaskDetailVo.taskToken}`).then((text) => (console.log(text)))
              }
            }
          }
        } catch (e) {
