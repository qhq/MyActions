const $ = new Env(' 云闪付');
$.SESSION_KEY = 'qhq_ysf_keys';

!(async () => {
  if (!$.getdata($.SESSION_KEY)) {
    $.subt = ' 未找到 Keys';
    $.desc = ' 请获取根据说明获取 Keys，点击前往';
    $.msg($.name, $.subt, $.desc, {
      'open-url':
        'https://gitee.com/qhqcz/quan-x/raw/master/Scripts/ysf.cookie.js',
    });
...
function signin() {
  return new Promise((resolve) => {
    ...
    opts.url = 'https://youhui.95516.com/newsign/api/daily_sign_in';
...
function showmsg() {
  return new Promise((resolve) => {
    const { coins, days,signedIn } = $.resData;
    ...
    if (coins > 0 ) {
      $.subt = ' 签到成功';
      $.desc = ` 今日获得 ${coins/100}  签到金；已累计签到 ${sum} 天`;
      $.msg($.name, $.subt, $.desc);
    }
    if (coins == 0) {
      $.subt = ' 签到失败';
      $.desc = ` 重复签到，已累计签到 ${sum} 天`;
      $.msg($.name, $.subt, $.desc);
    }
    resolve();
  });
