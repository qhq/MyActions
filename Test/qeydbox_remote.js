...
async function changeFiele(content, cookie) {
    ...
    content = content.replace(/^all\(\);/, "qqreadtask();")
    content = content.replace("todayAmount}金币\\n`;","todayAmount}金币\\n`;\nconsole.log(`【宝箱任务${task.data.treasureBox.count + 1}】:${task.data.treasureBox.tipText}\\n`);\nconsole.log(`距离时间：${task.data.treasureBox.timeInterval}ms`);\nsleep(`${task.data.treasureBox.timeInterval}` == null ? 0 : `${task.data.treasureBox.timeInterval}`);")
